/**
 * Server-side OpenRouter API integration using the OpenAI API format
 */
...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    "HTTP-Referer": "https://swipefolio.replit.app", // Replace with your actual domain
    "X-Title": "SwipeFolio"
  }
});

/**
 * Create a prompt for OpenRouter to generate stock information for a specific stack
 */
...
/**
 * Call the OpenRouter API to generate stock information
...
export async function generateAIStockData(stackName: string): Promise<any> {
  try {
    ...
    const prompt = createOpenRouterPrompt(stackName);
    
    // Use the gpt-4 model via OpenRouter
    ...
    const stockDataText = response.choices[0].message.content;
    if (!stockDataText) {
    ...
    try {
      ...
      console.error("Failed to parse JSON from response:", stockDataText);
 ...
 * @param stockData The full stock data object from OpenRouter
 */
